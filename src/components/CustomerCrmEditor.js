...
export default function CustomerCrmEditor({ phone, initialTags, initialNote }) {
  ...
  return (
    <div className="card">
      <h2 className="section-title" style={{ marginTop: 0 }}>🏷️ Tags & Notes</h2>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '12px' }}>
        {tags.map(tag => (
          <span key={tag} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '4px 10px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: '700', backgroundColor: 'rgba(193,68,14,0.1)', color: 'var(--primary-terracotta)' }}>
            {tag}
            <button onClick={() => removeTag(tag)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', fontWeight: '700', padding: 0 }}>✕</button>
          </span>
        ))}
      ...
      <button onClick={handleSave} disabled={saving} className="btn btn-primary" style={{ width: '100%' }}>
        {saving ? 'Saving...' : saved ? '✅ Saved' : 'Save Tags & Note'}
      </button>
    </div>
  );
}
