export default function InvoiceDocument({ order, invoice, business }) {
  ...
  // Product prices are entered GST-inclusive (see Settings > GST) — this
  // backs the tax amount out of the order total for display rather than
  // adding anything on top of what the customer paid. Shown as one combined
  // GST line, not a CGST/SGST/IGST split (that needs a structured
  // customer-state field this app doesn't collect yet).
  const showGst = business.gstRegistered && Number(business.gstRatePercent) > 0;
  const gstRate = showGst ? Number(business.gstRatePercent) : 0;
  const taxableValue = showGst ? order.totalAmount / (1 + gstRate / 100) : order.totalAmount;
  const gstAmount = showGst ? order.totalAmount - taxableValue : 0;

  return (
    <div className="invoice-area" style={{ fontFamily: 'var(--font-ui)', color: '#000', backgroundColor: 'white' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '48px' }}>
        <div style={{ width: '300px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', borderBottom: '1px solid #eee' }}>
            <span>{showGst ? 'Taxable Value:' : 'Subtotal:'}</span>
